...
from huggingface.transformers import BertTokenizer, BertForMaskedLM, BasicTokenizer, BertModel
import numpy as np
...
class Bert(Base_Connector):
    ...
    def get_batch_generation(self, sentences_list, labels_id_list, logger= None,
                             try_cuda=True):
        ...
        #Define labels to compute loss for saliency
        labels = tokens_tensor.clone()
        labels[labels != self.inverse_vocab[MASK]] = -1 #per old documentation, -1s are ignored
        labels[labels == self.inverse_vocab[MASK]] = labels_id_list
        print('\n Labels:', labels)

        # tokens_tensor = tokens_tensor.to(self._model_device)
        # tokens_tensor.requires_grad_() Only float tensors can require grads
        #####################################
        #Saliency computation using the loss
        #####################################
        self.masked_bert_model.zero_grad()
        loss = self.masked_bert_model(
            input_ids=tokens_tensor.to(self._model_device),
            token_type_ids=segments_tensor.to(self._model_device),
            attention_mask=attention_mask_tensor.to(self._model_device),
            masked_lm_labels = labels.to(self._model_device)
        )

        print('\n Loss: ', loss)
        #Compute naive saliency (use create_graph = True for finding the input tokens most responsible for high saliency later)
        gradients = autograd.grad(loss, self.masked_bert_model.parameters(), create_graph=False, allow_unused = True)
        # loss.backward()
        ...
        # for param in self.masked_bert_model.parameters():
        #     naive_saliency.append(param.grad.view(-1).detach().cpu())

        naive_saliency = torch.abs(torch.cat(naive_saliency))

        print('\n Saliency shape:', naive_saliency.shape)
        with torch.no_grad():
        ...
        return log_probs, token_ids_list, masked_indices_list, naive_saliency

    def get_batch_generation_old(self, sentences_list, logger=None,
                             try_cuda=True):
        """This is the old batch_generation function (with no loss computation and no saliency profile)"""
        if not sentences_list:
            return None
        if try_cuda:
            self.try_cuda()
